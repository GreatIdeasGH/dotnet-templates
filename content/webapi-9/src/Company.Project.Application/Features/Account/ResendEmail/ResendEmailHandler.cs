...
{
    ValueTask<ErrorOr<ApiResponse>> ResendEmail(
...
) : IResendEmailHandler
{
    ...
    public async ValueTask<ErrorOr<ApiResponse>> ResendEmail(
        ResendEmailRequest request,
        CancellationToken cancellationToken
    )
    {
        ...
        try
        {
            ...
            await publishEndpoint.Publish(
                new ResendEmailEvent(
                    response.Value.UserId,
                    response.Value.Email,
                    response.Value.VerificationCode
                ),
                cancellationToken
            );
        ...
        catch (Exception exception)
        {
            return exception.LogCriticalUser(
                ...
                activity: activity,
                user: request.Email,
                message: "Could not resend email confirmation"
            );
        }
    }
}
