    ...
    : IAuditRepository
{
    ...
    private IQueryable<AuditTrail> FilterAudits(AuditPagingParameters pagingParameters)
    {
        ...
        // Filter
        if (!string.IsNullOrWhiteSpace(pagingParameters.Name))
        {
            var param = pagingParameters.Name.Trim().ToLower();
            collections = collections.Where(m => m.Username.ToLower().Equals(param));
        }

        if (!string.IsNullOrWhiteSpace(pagingParameters.Action))
        {
            var param = pagingParameters.Action.Trim().ToLower();
            collections = collections.Where(m => m.Action.ToLower().Equals(param));
        }

        if (!string.IsNullOrWhiteSpace(pagingParameters.TableName))
        {
            var param = pagingParameters.TableName.Trim().ToLower();
